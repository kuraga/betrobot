...
from betting.proposers.corners_periods_results_proposers import CornersFirstPeriodResults1Proposer, CornersFirstPeriodResults1XProposer, CornersFirstPeriodResultsX2Proposer, CornersFirstPeriodResults2Proposer, CornersSecondPeriodResults1Proposer, CornersSecondPeriodResults1XProposer, CornersSecondPeriodResultsX2Proposer, CornersSecondPeriodResults2Proposer
...
for train_sampler_name, train_sampler in train_samplers.items():
    ...
    provider1 = Provider(name, description, train_sampler, corners_attack_defense_fitter, corners_attack_defense_predictor, corners_results_proposers_data)
    make_experiment(provider1, db_name, matches_collection_name, sample_condition)


    name = 'provider-corners_periods_results-corners_periods_attack_defense-%s' % (train_sampler_name,)
    description = 'Исходы угловых 1-го тайма и 2-го тайма, предсказание по атаке и обороне команд (угловые, рассматривается вероятность счетов), недавние данные'
    provider2 = Provider(name, description, train_sampler, corners_attack_defense_fitter, corners_attack_defense_predictor, corners_periods_results_proposers_data)
