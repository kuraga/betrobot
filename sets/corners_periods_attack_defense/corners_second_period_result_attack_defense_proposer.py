...
from sport_util import is_betarch_match_corner, count_events_of_teams, is_corner, is_second_period
...
class CornersSecondPeriodResultAttackDefenseProposer(TeamsPairAndTournnamentBasedProposer):

    def __init__(self, betting_sessions=None, model_name_pattern='corners_second_period-attack_defense-%d'):
    ...
    def propose(self, betcity_match, whoscored_match=None, tresholds=None):
        ...
        (corners_second_period_predicted_home, corners_second_period_predicted_away) = prediction 
        ...
        if corners_second_period_predicted_home - corners_second_period_predicted_away >= 1:
            if whoscored_match is not None:
                (corners_second_period_home_count, corners_second_period_away_count) = count_events_of_teams(lambda event: is_corner(event) and is_second_period(event), whoscored_match)
                ground_truth = corners_second_period_home_count > corners_second_period_away_count
            else:
                ground_truth = None
