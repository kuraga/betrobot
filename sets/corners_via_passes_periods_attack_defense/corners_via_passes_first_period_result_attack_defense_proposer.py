...
class CornersViaPassesFirstPeriodResultAttackDefenseProposer(TeamsPairAndTournamentBasedProposer):

    def __init__(self, betting_sessions=None,
                 model_name_pattern=['crosses_first_period-attack_defense-%d', 'saved_shots_first_period-attack_defense-%d']):
    ...
    def propose(self, betcity_match, whoscored_match=None, tresholds=None):
        ...
        crosses_first_period_prediction = self._predict(betcity_match, model_index=0)
        if crosses_first_period_prediction is None:
            return
        # Наиболее вероятные индивидуальные тоталы кроссов в первом тайме
        (crosses_first_period_predicted_home, crosses_first_period_predicted_away) = crosses_first_period_prediction

        saved_shots_first_period_prediction = self._predict(betcity_match, model_index=1)
        if saved_shots_first_period_prediction is None:
            return
        # Наиболее вероятные индивидуальные тоталы кроссов в первом тайме
        (saved_shots_first_period_predicted_home, saved_shots_first_period_predicted_away) = saved_shots_first_period_prediction

        corners_first_period_predicted_home = int( (crosses_first_period_predicted_home * 0.09 + saved_shots_first_period_predicted_home * 0.22) / 0.6 )
        corners_first_period_predicted_away = int( (crosses_first_period_predicted_away * 0.09 + saved_shots_first_period_predicted_away * 0.22) / 0.6 )
