...
from sport_util import is_betarch_match_main, count_events_of_teams, is_goal, is_second_period
    ...
    def propose(self, betcity_match, whoscored_match=None, tresholds=None):
        ...
        if goals_second_period_predicted_home - goals_second_period_predicted_away >= 1:
            if whoscored_match is not None:
                (goals_second_period_home_count, goals_second_period_away_count) = count_events_of_teams(lambda event: is_goal(event) and is_second_period(event), whoscored_match)
                ground_truth = goals_second_period_home_count > goals_second_period_away_count
            else:
                ground_truth = None
